  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 flex flex-col items-center">
      <img
        src="/Orest.png"
        alt="Orest"
        className="mb-6 rounded-full w-36 h-36 object-cover border-4 border-sky-300 shadow"
      />
      <div className="text-sky-800 font-serif">
        <span className="block text-2xl font-bold mb-3 underline decoration-sky-400">
          Дякую, що хоч розгадав код і зайшов сюда
        </span>
        <p className="text-lg italic leading-relaxed mb-4">
          Надіюсь, тобі сподобався цей невеликий подарунок. Я вклав у нього
          достатньо так часу душі, трохи гумору, аішки, способів шукання фото і
          можливостей пограти в доту/кс/ще щось.
        </p>
      </div>
      <a
...
const DotaStats = () => {
  ...
  const [sandKingGamesAndWins, setSandKingGamesAndWins] = useState({
    games: 0,
    wins: 0,
  })
  ...
  const fetchChatMessages = () => {
    return fetch(
      'https://api.opendota.com/api/players/342101363/heroes?hero_id=16'
    )
      .then((res) => {
        if (res.ok) {
          return res.json()
  ...
  useEffect(() => {
    Promise.all([fetchAvatar(), fetchWinRate(), fetchChatMessages()]).finally(
      () => setIsLoading(false)
    )
  }, [])
  ...
  const winrate =
    winLoss.loss > 0
      ? ((winLoss.win / (winLoss.win + winLoss.loss)) * 100).toFixed(1)
      : 'N/A'
  const sandKingWinrate =
    sandKingGamesAndWins.games > 0
      ? (
          (sandKingGamesAndWins.wins / sandKingGamesAndWins.games) *
          100
        ).toFixed(1)
      : 'N/A'
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 flex flex-col items-center">
      {isLoading ? (
        ...
        <>
          <img
            src={avatarFullURL}
            className="mb-6 rounded-full w-28 h-28 object-cover border-4 border-sky-300 shadow"
          />
          <div className="text-sky-800 font-serif text-center">
            <div className="text-xl font-semibold mb-2">
              Цікаві факти про тебе:
            </div>
            <ul className="list-disc list-inside text-lg mb-2">
              <li>
                Твій вінрейт:{' '}
                <span className="font-bold text-sky-600">{winrate}%</span>
              </li>
              <li>
                Ігор на Сенд Кінгу:{' '}
                <span className="font-bold text-sky-600">
                  {sandKingGamesAndWins.games}
                </span>
                , перемог:{' '}
                <span className="font-bold text-sky-600">
                  {sandKingGamesAndWins.wins}
                </span>{' '}
                ({sandKingWinrate}%)
              </li>
            </ul>
    ...
    <div className="bg-white rounded-2xl shadow-lg p-8 mb-8 flex flex-col items-center">
      ...
      <div className="text-sky-800 font-serif text-left">
        ...
        <p className="text-lg italic leading-relaxed mb-2">
          Почав я його робити десь в 20 числах липня, з перервою на 10 днів на
          геймджем, я весь час хотів розказати хоч Дімі, Марічці, чи Ляні, але
          це б їм спортило трохи сюрприз. Я старався аж настільки, що хочеться
          попросити вибачення в Марічки за такий маленький подарунок їй).
        </p>
        <p className="text-lg italic leading-relaxed mb-2">
          Насправді, поки я пишу цей текст, подарунок досі не готовий, ще
          залишилося дописати цей сайт, доробити трохи картки, замовити саму
          колоду, але надіюся встигну ^^.
        </p>
        <p className="text-lg italic leading-relaxed mb-2">
          Не знаю, чи захочу найближчим часом робити щось таке ж велике, але я
          радий, що я за це взявся і (поки що майже) довів справу до кінця, ну а
          ще сайт я захотів зробити чисто тому що якось зновау перед Марічкою
          вийшло, що вона мені трхи показувала і я щось на довстатньо довгий час
          забив на то.
        </p>
        <p className="text-lg italic leading-relaxed mb-2">
          Якщо ще з коротких побажань то фоткайтеся більше з Дімою, бо ваші
          фотки було не так і легко знайти зразу.
        </p>
        <p className="text-lg italic leading-relaxed">
          Мта, перечитуючи цей текст, є відчуття ніби це якийсь більш лист
          смертника і я мав би скоро зробити суїціид, ну хто зна, може колись
          таке і трапиться.
        </p>
      </div>
    </div>
...
const App = () => {
  ...
  useEffect(() => {
    const allowed = prompt('Enter password: (Шифр-13)') === String(password)
